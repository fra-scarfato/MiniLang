...
let get_block_used_vars block =
  List.concat_map get_used_vars_cmd block.stmts
...
(* Takes a list of commands and the current 'open' block.
   Returns the builder and the ID of the block where flow ends.
*)
let rec gen_stmts builder current_block_id cmds =
  ...
  | cmd :: rest ->
      ...
      | If (cond, then_branch, else_branch) ->
          (* 1. Put the 'If' stmt in the current block (as the terminator condition) *)
          let builder = append_stmt builder current_block_id (If (cond, Skip, Skip)) in
          ...
          let (builder, then_end) = gen_stmts builder then_entry (flatten_cmd then_branch) in
          ...
          let (builder, else_end) = gen_stmts builder else_entry (flatten_cmd else_branch) in
      ...
      | While (cond, body) ->
          ...
          let builder = add_edge builder current_block_id header_id Unconditional in
          ...
          let (builder, body_end) = gen_stmts builder body_entry (flatten_cmd body) in
          
          (* 5. Wire edges *)
          let builder = add_edge builder header_id body_entry True in   (* Loop matches *)
          let builder = add_edge builder header_id exit_id False in     (* Loop finishes *)
          let builder = add_edge builder body_end header_id Unconditional in (* Back edge *)
          
          (* 6. Continue in Exit block *)
          gen_stmts builder exit_id rest
      
      | Seq _ -> failwith "Should be flattened" (* Unreachable due to flatten_cmd *)

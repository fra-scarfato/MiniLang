...
let get_used_registers = function
  ...
  | Load (addr_reg, _) -> [addr_reg]    (* Reads the address register to fetch memory *)
  | Store (val_reg, addr_reg) -> [val_reg; addr_reg] (* Reads value to store and address *)
...
let uses_register reg instr =
  List.mem reg (get_used_registers instr)

let defines_register reg instr =
  List.mem reg (get_defined_registers instr)
...
let rec peephole = function
  ...
  | UnaryOp (Copy, r1, r2) :: rest when r1 = r2 ->
      peephole rest
...
let rec trans_op ctx = function
      ...
      if v = ctx.input_var then
        ...
        (match VarMap.find_opt v ctx.var_to_reg with
         ...
         | None ->
             ...
             let (ctx', r) = allocate_register ctx in
             let ctx'' = { ctx' with var_to_reg = VarMap.add v r ctx'.var_to_reg } in
             (ctx'', r, []))

  (* ===== ALGEBRAIC SIMPLIFICATION FOR PLUS ===== *)
  
  (* Identity: x + 0 = x *)
  | Plus (op, Constant 0) | Plus (Constant 0, op) -> 
      trans_op ctx op
  ...
  (* Identity: x * 1 = x *)
  | Times (op, Constant 1) | Times (Constant 1, op) -> 
      trans_op ctx op
  
  (* Annihilation: x * 0 = 0 *)
  | Times (_, Constant 0) | Times (Constant 0, _) ->
      trans_op ctx (Constant 0)
...
let rec trans_bool ctx = function
  ...
  | And (Bool false, _) | And (_, Bool false) ->
      trans_bool ctx (Bool false)
  
  | And (Bool true, b) | And (b, Bool true) ->
      trans_bool ctx b
...
let rec trans_cmd ctx = function
  ...
  | Assign (v, op) ->
      ...
      let (target_reg, ctx') = 
        if v = ctx.input_var then
          (input_register, { ctx with var_to_reg = VarMap.add v input_register ctx.var_to_reg })
        else if v = ctx.output_var then
          (output_register, { ctx with var_to_reg = VarMap.add v output_register ctx.var_to_reg })
        else
          (* Check if variable already has a register *)
          match VarMap.find_opt v ctx.var_to_reg with
          | Some existing_reg -> (existing_reg, ctx)
          | None -> (result_reg, { ctx with var_to_reg = VarMap.add v result_reg ctx.var_to_reg })
      in
...
let trans_cmd_list ctx cmds =
  let (ctx, acc_cmds, cond_reg) = 
    List.fold_left (fun (ctx, acc_cmds, _) cmd ->
...
let translate_cfg input_var output_var (imp_cfg : MiniImpCFG.cfg) : risc_cfg =
  ...
  let block_labels = 
    MiniImpCFG.BlockMap.mapi (fun id _ -> make_label id) imp_cfg.blocks in
  ...
  let (_, risc_blocks) =
    MiniImpCFG.BlockMap.fold (fun id imp_block (ctx, acc) ->
      ...
      let terminator = match successors with
        | [] -> None
    ...
    ) imp_cfg.blocks (ctx, MiniRISCCFG.BlockMap.empty)
  in
  
  (* Convert edges from MiniImpCFG to MiniRISCCFG format *)
  let risc_edges = 
    MiniImpCFG.BlockMap.fold (fun src edge_list acc ->
      let converted_edges = List.map (fun (dst, lbl) ->
        match lbl with
  ...
  { blocks = risc_blocks; edges = risc_edges; entry = imp_cfg.entry; exit = imp_cfg.exit }
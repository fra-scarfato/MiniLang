...
let string_of_command = function
  ...
  | BinRegOp (op, r1, r2, r3) -> 
      Printf.sprintf "%s %s %s => %s" 
        (string_of_brop op) (string_of_register r1) (string_of_register r2) (string_of_register r3)
  | BinImmOp (op, r, n, rd) ->
      Printf.sprintf "%s %s %d => %s"
        (string_of_biop op) (string_of_register r) n (string_of_register rd)
  | UnaryOp (op, r1, r2) ->
      Printf.sprintf "%s %s => %s"
        (string_of_urop op) (string_of_register r1) (string_of_register r2)
  | Load (r1, r2) ->
      Printf.sprintf "load %s => %s" (string_of_register r1) (string_of_register r2)
  | LoadI (n, r) ->
      Printf.sprintf "loadi %d => %s" n (string_of_register r)
  | Store (r1, r2) ->
      Printf.sprintf "store %s => %s" (string_of_register r1) (string_of_register r2)
  | Jump lbl ->
      Printf.sprintf "jump %s" (string_of_label lbl)
  | CJump (r, ltrue, lfalse) ->
      Printf.sprintf "cjump %s %s %s" 
        (string_of_register r) (string_of_label ltrue) (string_of_label lfalse)

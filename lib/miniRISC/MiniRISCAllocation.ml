 ...
 * have limited registers - maybe only 8 or 16. We need to:
 *
 *   1. Decide which virtual registers get physical registers (the fast path)
 *   2. Decide which virtual registers must live in memory (the slow path)
 ...
 *
 * 1. INTERFACE STABILITY: External code expects to find input/output in these
 *    specific registers. If we merge r_out with r5, then spill r5 to memory,
 *    suddenly the output isn't where it should be!
 *
 * 2. SIMPLICITY: Treating them as sacred means we never have edge cases like
 ...
 *
 * 3. MISSED OPTIMIZATIONS ARE ACCEPTABLE: Yes, we might miss chances to
 ...
 *
 * This is a classic engineering trade-off: simplicity and correctness over
 * squeezing out every last micro-optimization.
 *)

(* Instruction point for fine-grained liveness *)
type instr_point = int * int

module InstrPointSet = Set.Make (struct
  type t = instr_point
  let compare = compare
end)
...
let compute_live_ranges_from_liveness liveness_result =
  Dataflow.LiveVariables.InstrPointMap.fold
    (fun point live_set acc ->
      RegisterSet.fold
        (fun reg range_map ->
          ...
          in
          RangeMap.add reg (InstrPointSet.add point current_points) range_map
        )
        live_set acc
    )
    liveness_result.Dataflow.LiveVariables.instr_after
    RangeMap.empty

(* Check if two live ranges interfere (have overlapping instruction points) *)
let ranges_interfere range1 range2 =
  not (InstrPointSet.is_empty (InstrPointSet.inter range1 range2))

(* Union two live ranges *)
let union_ranges range1 range2 =
  InstrPointSet.union range1 range2
...
let merge_registers ?(verbose = false) cfg =
  ...
  let all_regs =
    ...
    |> List.map fst 
    |> List.filter (fun r -> r <> input_register && r <> output_register)  (* r_in and r_out are never merged *)
    |> List.sort compare
  in

  log_verbose verbose "Total virtual registers (excluding r_in/r_out): %d" (List.length all_regs);
        ...
        let valid_group =
          RangeMap.fold
            (fun rep_reg group_range acc ->
              ...
              | Some _ -> acc 
              | None ->
                  (* Check: Ranges must not interfere *)
  ...
  log_verbose verbose "Note: r_in and r_out are never merged (always kept as special registers)";
      ...
      BlockMap.map
        (fun block ->
          ...
          let rename_cmd = function
            ...
            | UnaryOp (op, r1, rd) ->
                UnaryOp (op, map_lookup r1, map_lookup rd)
...
(* =============================================================================
 * STEP 2: ALLOCATION & SPILLING (The Hard Part)
 * =============================================================================
 *
...
(* Memory address base - offsets addresses to distinguish them from regular integers *)
...
let allocate_locations ?(verbose = false) cfg n_target =
  ...
  if n_target < 4 then failwith "Target must have >= 4 registers (r_in, r_out, r_a, r_b)";
  ...
  *)
  ...
  log_verbose verbose "    - r_in, r_out: 2 registers (I/O, always in registers)";
  log_verbose verbose "    - r_a, r_b: 2 registers (swap, for spilling operations)";
  log_verbose verbose "    - General-purpose: %d registers (for program variables)" available_slots;
   ...
   * ---------------------------------------------------------------------------
   *
   * We have more virtual registers than physical slots. Time to make hard
   * choices. Our strategy:
   *
   ...
   *
   * WHY +2 IN SLOT NUMBERING?
   * Physical registers are numbered starting from 0, but r0=r_a and r1=r_b
   * are reserved for swapping. So the first program register gets slot 2.
   *
   ...
   *)
  (* Get program registers (exclude special I/O and swap registers) *)
  let sorted_bindings =
  ...
  (* Allocation: most frequent variables get physical registers, rest go to memory *)
    ...
    List.fold_left
      (fun (map, next_addr) (idx, reg) ->
        if idx < available_slots then (
          ...
          log_verbose verbose "  [Reg] %s (freq=%d)" 
            (string_of_register reg) (RegisterMap.find reg freqs);
          ...
          log_verbose verbose "  [Mem@0x%x] %s (freq=%d)" 
            mem_addr (string_of_register reg) (RegisterMap.find reg freqs);
          ( RegisterMap.add reg (InMemory mem_addr) map,
            next_addr + 1 )
        ))
      (RegisterMap.empty, 0)
  ...
  log_verbose verbose "Summary: %d in registers, %d spilled" 
    (min (List.length sorted) available_slots) next_mem_addr;
  alloc_map
(* =============================================================================
...
let rewrite_block alloc_map block =
  let rewrite_cmd = function
    | BinRegOp (op, r1, r2, rd) ->
        ...
        let op_dest, stores = match rd_alloc with
          | Some (InRegister phys_reg) ->
              (* Destination is in a physical register, use it directly *)
              (phys_reg, [])
          | Some (InMemory _) ->
              (* Destination is spilled: compute to r_b, then store using r_a for address *)
    ...
    | Load (addr, rd) ->
        ...
        let op_dest, stores = match rd_alloc with
          | Some (InRegister phys_reg) -> (phys_reg, [])
          | Some (InMemory _) ->
              (* addr uses r_a, so use r_b for result, then r_a for store address *)
              (r_b, store_if_needed alloc_map rd r_b r_a)
          | None -> (rd, [])
        in
...
let report_memory_allocations ?(verbose = false) alloc_map =
  ...
  if verbose && memory_regs <> [] then (
    ...
    List.iter
      (fun (reg, addr) ->
        log_verbose verbose "  %s -> 0x%x" 
          (string_of_register reg) addr)
      memory_regs
  );
...
let allocate_registers ?(optimize = false) ?(verbose = false) n_target cfg =
  log_verbose verbose "\n=== REGISTER ALLOCATION (n=%d, optimize=%b) ===" n_target optimize;

  (* STEP 1: Register Merging (Optional Optimization) *)
  let merged_cfg =
    if optimize then merge_registers ~verbose cfg
    else cfg
  in
  ...
  (* STEP 3: Code Rewriting with Spilling *)
  log_verbose verbose "\n=== STEP 3: CODE REWRITING ===";

...
let linearize_cfg (cfg : risc_cfg) : risc_program =
  ...
  let rec emit_blocks = function
    ...
    | block :: rest ->
        ...
        let label = 
          if block.id = cfg.entry then Label "main"
          else block.label
        in
        
        let label_def = [LabelDef label] in
        let instrs = List.map (fun cmd -> Instruction cmd) block.commands in
        let term = match block.terminator with
          | Some t -> [Instruction t]
          | None -> []
        in
...
let program_to_string prog =
  prog
  |> List.map string_of_labeled_instruction
  |> String.concat "\n"

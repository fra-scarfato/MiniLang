...
let parse_arguments () =
  ...
  if argc = 4 && Sys.argv.(3) = "log" then set_logging true;
...
let main () =
  ...
  log_message "[Interpreter]" "Starting evaluation";

  (* Read and parse the program from the specified file *)
  let program = read_program in_file_name in

  (* Build the CFG from the parsed program *)
  let cfg = MiniImpCFG.build_cfg_from_program program in

  (* Evaluate the program with the provided input integer *)
  let result = MiniImpEval.eval_program input_int program in

  (* Print the CFG and evaluation result *)
  print_string (MiniImpCFG.string_of_cfg cfg);
  Printf.printf "The result of the eveluation is: %i\n" result;
  log_message "[Interpreter]" "Evaluation finished"

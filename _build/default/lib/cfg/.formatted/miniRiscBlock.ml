...
(** This module defines blocks for MiniRisc programs. A block represents a basic
    block in the control flow graph. *)

module MiniRiscBlock = struct
  type block = { label : label; commands : command list; term : command }
  (** [block] represents a block in a MiniRisc program. It consists of:
